...
The PostgreSQL verification sequence uses a guarded disposable test database. It can target either the repository test service or an already-running local PostgreSQL 17 service; Docker is not required when the latter is available:
...
npm run cli -- recommend --config-dir config --limit 20
```
...
Chunk 4 or Chunk 5 migration state, create a fresh guarded test database, apply migrations
