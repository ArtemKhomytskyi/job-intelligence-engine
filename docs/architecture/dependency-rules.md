...
For configuration, application owns `ConfigFileReader` and `ConfigurationDecoder`. Infrastructure may import Zod, YAML, and Node filesystem APIs to implement them. Domain must not import those technologies. CLI modules compose adapters and format output but must not parse YAML or define schemas.

For persistence, application owns repositories and `PersistenceTransactionManager`. Infrastructure may import Prisma and map generated records, enums, Decimal, JSON, and dates. Application/domain modules must not import Prisma, and repositories must not return raw records. CLI database commands compose health/migration adapters and sanitize failures.
